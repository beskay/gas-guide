...
contract OriginalGarageSocialClub is Ownable, ERC721Enumerable {
    ...
    function getPrice(uint256 _classId) public view returns (uint256) {
        ...
        if (classInfo.priceType == 1) {
            ...
            if (classInfo.startPrice > classInfo.endPrice) {
                // dutch price goes down over time
                mintPrice = classInfo.endPrice
                    + (
                        (classInfo.startPrice - classInfo.endPrice) * (classInfo.endTime - block.timestamp)
                            / (classInfo.endTime - classInfo.startTime)
                    );
            } else {
                // dutch price goes up over time
                mintPrice = classInfo.startPrice
                    + (
                        (classInfo.endPrice - classInfo.startPrice) * (block.timestamp - classInfo.startTime)
                            / (classInfo.endTime - classInfo.startTime)
                    );
            }
        }
    ...
    function mint(uint256 _classId, uint256 _amount, address _to) public payable {
        ...
        require(
            amountMintedForClass + _amount <= classInfo.maxSupply, "mint: Not enough supply remaining of this class"
        );
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal {
        ERC721Enumerable._beforeTokenTransfer(from, to, tokenId, 0);
    }
